...
for i, p, theta in zip(range(m1), p, theta):
    ...
    data[i+131, :] = np.concatenate(([14], time_series), axis=0)
...
for i, a, b, c in zip(range(5), a, b, c):
    ...
    x, y, z = time_series[:, 0], time_series[:, 1], time_series[:, 2]
    data[i+151, :] = np.concatenate(([16], x), axis=0)
    data[i+156, :] = np.concatenate(([16], y), axis=0)
    data[i+161, :] = np.concatenate(([16], z), axis=0)
